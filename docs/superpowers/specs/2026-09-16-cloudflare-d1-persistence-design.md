...
- `public/login.html` asks for a first name and the passphrase. `POST /api/login` compares the passphrase in constant time and, on success, sets a signed, HttpOnly, Secure, SameSite=Lax cookie that lasts one year. The cookie carries the name and an expiry, signed with HMAC-SHA256. A wrong passphrase is answered after a one-second delay with a generic error. The delay only slows a single guesser; parallel guesses are not throttled, so the passphrase's length is the real protection and the README asks for a full sentence. A Cloudflare rate-limiting rule on `/api/login` is the upgrade if that ever matters.
